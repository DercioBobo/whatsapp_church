...
def get_doctype_fields(doctype):
    ...
    try:
        ...
        valid_fieldtypes = [
            ...
            "Currency", "Check", "Date", "Datetime", "Time", "Table"
        ]
...
def _get_group_config_for_doctype(doctype):
    """Return the WhatsApp Group DocType Config row for a given doctype, or None."""
    try:
        settings = frappe.get_single("Evolution API Settings")
        for row in (settings.group_doctypes or []):
...
def collect_phones_for_group(doctype, docname):
    """
    Collect all phone numbers from a doc's child table, as configured in
    Evolution API Settings → Group Creation DocTypes.

    Handles:
    - num1/num2/num3 format in the phone field
    - Fallback to a linked doc's 'contacto' if phone field is empty

    Returns:
        list[str]: Formatted phone numbers (e.g. ["258841234567", ...])
    """
    ...
    config = _get_group_config_for_doctype(doctype)
    if not config or not config.child_table_field or not config.phone_field:
        return []

    # Strip display label suffix if present (fieldname  —  Label → fieldname)
    child_table_field = config.child_table_field.split("  —  ")[0].strip()
    phone_field = config.phone_field.strip()
    fallback_link_field = (config.fallback_link_field or "").strip()
    ...
    # Pre-resolve the fallback linked doctype (once, not per row)
    fallback_linked_doctype = None
    if fallback_link_field:
        try:
            parent_meta = frappe.get_meta(doctype)
            table_field_def = parent_meta.get_field(child_table_field)
            if table_field_def and table_field_def.options:
                child_meta = frappe.get_meta(table_field_def.options)
                link_field_def = child_meta.get_field(fallback_link_field)
                if link_field_def and link_field_def.options:
                    fallback_linked_doctype = link_field_def.options
        except Exception:
            pass

    for row in (getattr(doc, child_table_field, None) or []):
        raw = getattr(row, phone_field, None)

        # Fallback: look up linked doc's contacto
        if not raw and fallback_link_field and fallback_linked_doctype:
            link_name = getattr(row, fallback_link_field, None)
            if link_name:
                try:
                    linked_doc = frappe.get_doc(fallback_linked_doctype, link_name)
                    raw = getattr(linked_doc, "contacto", None) or getattr(linked_doc, phone_field, None)
                except Exception:
                    pass
...
def whatsapp_group_create(group_name, phones):
    """
    Create a WhatsApp group via Evolution API.

    Args:
        group_name: Group subject/name
        phones: List of formatted phone numbers (e.g. ["258841234567"])

    Returns:
        dict: {success, group_id, error}
    """
    from whatsapp_notifications.whatsapp_notifications.doctype.evolution_api_settings.evolution_api_settings import get_settings

    settings = get_settings()
    if not settings.get("enabled"):
        return {"success": False, "error": _("WhatsApp disabled")}

    url = "{}/group/create/{}".format(settings["api_url"], settings["instance_name"])
    ...
    try:
        ...
        if response.status_code in (200, 201):
            group_id = data.get("id") or data.get("groupJid") or ""
            return {"success": True, "group_id": group_id, "data": data}
        return {"success": False, "error": "API {}: {}".format(response.status_code, data)}
    except Exception as e:
        return {"success": False, "error": str(e)}


def whatsapp_group_add_participants(group_id, phones):
    """
    Add participants to an existing WhatsApp group via Evolution API.

    Args:
        group_id: WhatsApp group JID
        phones: List of formatted phone numbers
    ...
    url = "{}/group/updateParticipant/{}".format(settings["api_url"], settings["instance_name"])
    headers = {"Content-Type": "application/json", "apikey": settings["api_key"]}
    payload = {
        "groupJid": group_id,
        "action": "add",
...
@frappe.whitelist()
def get_group_doctypes_list():
    """Return list of DocType names configured for group creation."""
    try:
        settings = frappe.get_single("Evolution API Settings")
        doctypes = [row.document_type for row in (settings.group_doctypes or []) if row.document_type]
        return {"success": True, "doctypes": doctypes}
    except Exception as e:
        return {"success": False, "doctypes": [], "error": str(e)}


@frappe.whitelist()
def get_group_phones_preview(doctype, docname):
    """Preview phone numbers that would be added to the group for a given doc."""
    phones = collect_phones_for_group(doctype, docname)
    return {"success": True, "phones": phones, "count": len(phones)}


@frappe.whitelist()
def get_grupo_for_doc(doctype, docname):
    """Return the WhatsApp Grupo record name linked to a doc, if any."""
    existing = frappe.get_all(
        "WhatsApp Grupo",
        filters={"source_doctype": doctype, "source_docname": docname, "status": "Ativo"},
        fields=["name", "grupo_nome", "grupo_id", "member_count"],
        limit=1
    )
    if existing:
        return {"found": True, "grupo_name": existing[0].name,
                "grupo_nome": existing[0].grupo_nome,
                "grupo_id": existing[0].grupo_id,
                "member_count": existing[0].member_count}
    return {"found": False}


@frappe.whitelist()
def create_or_update_grupo(doctype, docname, grupo_nome, grupo_whatsapp_name=None):
    """
    Create a new WhatsApp group for a doc, or update members of an existing one.

    Args:
        doctype: Source DocType
        docname: Source document name
        grupo_nome: Group display name
        grupo_whatsapp_name: Existing WhatsApp Grupo doc name (for update flow)

    Returns:
        dict: {success, grupo_name, group_id, count, created}
    """
    phones = collect_phones_for_group(doctype, docname)
    if not phones:
        return {"success": False, "error": _("Nenhum telefone encontrado no documento.")}

    if grupo_whatsapp_name:
        # UPDATE: add current members to existing group
        grupo_doc = frappe.get_doc("WhatsApp Grupo", grupo_whatsapp_name)
        result = whatsapp_group_add_participants(grupo_doc.grupo_id, phones)
        if not result.get("success"):
            return result
        grupo_doc.member_count = len(phones)
        grupo_doc.last_synced = frappe.utils.now_datetime()
        grupo_doc.save(ignore_permissions=True)
        frappe.db.commit()
        return {
            "success": True, "created": False,
            "grupo_name": grupo_doc.name,
            "group_id": grupo_doc.grupo_id,
            "count": len(phones)
        }
    else:
        # CREATE: create new group on WhatsApp
        result = whatsapp_group_create(grupo_nome, phones)
        if not result.get("success"):
            return result

        group_id = result.get("group_id", "")
        grupo_doc = frappe.get_doc({
            "doctype": "WhatsApp Grupo",
...
@frappe.whitelist()
def get_all_grupos():
    """Return all active WhatsApp Grupo records for use in Aviso fonte picker."""
    grupos = frappe.get_all(
        "WhatsApp Grupo",
        filters={"status": "Ativo"},
        fields=["name", "grupo_nome", "grupo_id", "member_count", "source_doctype", "source_docname"],
        order_by="grupo_nome asc"
    )
