...
frappe.whatsapp.open_table_helper = function() {
    let d = new frappe.ui.Dialog({
        ...
        fields: [{
            ...
            options: `
<div class="wa-th-wrap" style="padding: 4px 0;">
...
function _wa_build_table(raw, has_header) {
    ...
    // Auto-detect separator: tab → comma → pipe → single column
    let sep = '\t';
    let sample = lines[0];
    if (sample.indexOf('\t') === -1) {
        if (sample.indexOf(',') !== -1) sep = ',';
        else if (sample.indexOf('|') !== -1) sep = '|';
        else sep = null; // single column
    }
    ...
    // Calculate per-column max width
