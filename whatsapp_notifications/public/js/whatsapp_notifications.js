...
frappe.whatsapp._group_doctypes_cache = null;
frappe.whatsapp._group_doctypes_loading = false;

frappe.whatsapp.load_group_doctypes = function(callback) {
    if (frappe.whatsapp._group_doctypes_cache) {
        if (callback) callback(frappe.whatsapp._group_doctypes_cache);
        return;
    }
    if (frappe.whatsapp._group_doctypes_loading) {
        setTimeout(function() { frappe.whatsapp.load_group_doctypes(callback); }, 100);
        return;
    }
    frappe.whatsapp._group_doctypes_loading = true;
    frappe.call({
        method: 'whatsapp_notifications.whatsapp_notifications.api.get_group_doctypes_list',
        async: true,
        callback: function(r) {
            frappe.whatsapp._group_doctypes_loading = false;
            frappe.whatsapp._group_doctypes_cache = (r.message && r.message.success)
                ? (r.message.doctypes || []) : [];
            if (callback) callback(frappe.whatsapp._group_doctypes_cache);
        }
    });
...
frappe.whatsapp.maybe_add_group_button = function(frm) {
    if (frm.is_new()) return;

    frappe.whatsapp.load_group_doctypes(function(doctypes) {
        if (!doctypes.includes(frm.doctype)) return;

        frappe.call({
            method: 'whatsapp_notifications.whatsapp_notifications.api.get_grupo_for_doc',
            args: { doctype: frm.doctype, docname: frm.docname },
            async: true,
            callback: function(r) {
                let existing = r.message && r.message.found ? r.message : null;
                let btn_label = existing
                    ? __('Actualizar Grupo WhatsApp')
                    : __('Criar Grupo WhatsApp');

                frm.remove_custom_button(__('Criar Grupo WhatsApp'), __('Actions'));
                frm.remove_custom_button(__('Actualizar Grupo WhatsApp'), __('Actions'));

                frm.add_custom_button(btn_label, function() {
                    frappe.whatsapp.show_group_dialog(frm, existing);
                }, __('Actions'));
            }
        });
    });
...
frappe.whatsapp.show_group_dialog = function(frm, existing) {
    // First fetch preview of phone numbers
    frappe.call({
        method: 'whatsapp_notifications.whatsapp_notifications.api.get_group_phones_preview',
        args: { doctype: frm.doctype, docname: frm.docname },
        freeze: true,
        freeze_message: __('A carregar membros...'),
        callback: function(r) {
            let phones = (r.message && r.message.phones) || [];
            let count = phones.length;

            let default_name = existing
                ? existing.grupo_nome
                : (frm.doc.titulo || frm.doc.name || frm.docname);

            let dialog = new frappe.ui.Dialog({
                title: existing ? __('Actualizar Grupo WhatsApp') : __('Criar Grupo WhatsApp'),
                fields: [
                    {
                        fieldname: 'grupo_nome',
                        fieldtype: 'Data',
                        label: __('Nome do Grupo'),
                        reqd: 1,
                ...
                primary_action_label: existing ? __('Actualizar') : __('Criar Grupo'),
                primary_action: function(values) {
                    dialog.get_primary_btn().prop('disabled', true).html(__('A processar...'));
                    frappe.call({
                        method: 'whatsapp_notifications.whatsapp_notifications.api.create_or_update_grupo',
                        args: {
                            doctype: frm.doctype,
                            docname: frm.docname,
                            grupo_nome: values.grupo_nome,
                            grupo_whatsapp_name: existing ? existing.grupo_name : null
                        },
                        callback: function(r) {
                            if (r.message && r.message.success) {
                                dialog.hide();
                                let msg = r.message.created
                                    ? __('Grupo criado com {0} membros!', [r.message.count])
                                    : __('Grupo actualizado: {0} membros adicionados.', [r.message.count]);
                                frappe.show_alert({ message: msg, indicator: 'green' }, 6);
                                // Invalidate cache so button label refreshes
                                frappe.whatsapp._group_doctypes_cache = null;
                                frm.reload_doc();
                            } else {
                                dialog.get_primary_btn().prop('disabled', false)
                                    .html(existing ? __('Actualizar') : __('Criar Grupo'));
                                frappe.msgprint({
                                    title: __('Erro'),
                                    indicator: 'red',
                                    message: (r.message && r.message.error) || __('Falha na operação')
                                });
                            }
                        }
                    });
            ...
            let preview_lines = phones.slice(0, 10).map(function(p) {
                return '<div style="font-family:monospace;font-size:12px;">' + p + '</div>';
            }).join('');
            let more = count > 10 ? '<div style="color:#888;font-size:12px;">... e mais ' + (count - 10) + '</div>' : '';
            let existing_info = existing
                ? '<div style="margin-bottom:8px;padding:6px 10px;background:#e8f5e9;border-radius:6px;font-size:12px;">'
                  + '&#128994; Grupo existente: <strong>' + frappe.utils.escape_html(existing.grupo_nome) + '</strong>'
                  + ' (' + (existing.member_count || '?') + ' membros actuais)'
                  + '</div>' : '';
            ...
            dialog.fields_dict.preview_html.$wrapper.html(
                ...
                + (count > 0
                    ? '<strong>' + count + ' telefone(s)</strong> encontrado(s) para '
                      + (existing ? 'adicionar ao grupo:' : 'o grupo:')
                      + '<div style="margin-top:6px;background:#f5f6f7;border-radius:6px;padding:8px 12px;max-height:160px;overflow-y:auto;">'
                      + preview_lines + more + '</div>'
                    : '<span style="color:#d32f2f;">&#9888; Nenhum telefone encontrado. Verifique a configuração em Evolution API Settings.</span>')
                + '</div>'
            );
...
// Hook into form refresh to add WhatsApp media button and group button
$(document).on('form-refresh', function(e, frm) {
    if (frm && !frm.is_new()) {
        frappe.whatsapp.maybe_add_media_button(frm);
        frappe.whatsapp.maybe_add_group_button(frm);
    }
});
